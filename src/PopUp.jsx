...
const PopUp = ({title, message, closePopUp}) => {
  <div className='popup-outer'>
    <div className='popup-inner'>
      <IoIosWarning size={40} className='popup-image'/>
      <h2 className='popup-title'>{title}</h2>
      <p className='popup-message'>{message}</p>
      <button className='popup-close-button-outer' onClick={closePopUp}>
        <IoCloseOutline size={24} className='popup-close-button-inner'/>
       </button>
    </div>
  </div>
}
